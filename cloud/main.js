...
const cronAsset = async () => {
    ...
    qCertificateLog.equalTo("warning_date", new Date(new Date().toLocaleDateString()));
    ...
    certificates && certificates.forEach((item) => {
        ...
        const reminder = new Date((new Date()).setDate(current.getDate() + certificate_reminder))
        ...
        if(expdatetime && reminder && (expdatetime < reminder)){
            ...
            if(expdatetime < current) {
                listAlreadyExpiredCertificates.push(`<li>Name: ${name}, Expired Date: ${expdatetime.toDateString()}.</li>`);
            } else {
                listTobeExpiredCertificates.push(`<li>Name: ${name}, Expired Date: ${expdatetime.toDateString()}.</li>`);
            }
    ...
    fs.readFile(__dirname + '/templates/alertCertificate.html', 'utf8', (err, html) => 
    {
        ...
        certificateLog.set("warning_date", new Date(new Date().toLocaleDateString()));
...
Parse.Cloud.define("generate_contact_number", async (request) => {
    ...
    let contact = await qContact.get(objectId).catch((err) => { results.error = err});
    ...
    if(!contact.number || contact.number === "" || true)
    {
        ...
        const masterNumber2 = await qMasterNumber.get("7dTSFKSZ9x").then((mn) => {
...
Parse.Cloud.define("generate_wo_number", async (request) => {
    ...
    qTicket.select(["requester.department", "requester.firstname", "requester.lastname", "requester.user.username"]);
    qTicket.select(["assignto.department", "assignto.firstname", "assignto.lastname" , "assignto.user.username"]);
    qTicket.select(["createdby.firstname", "createdby.lastname", "createdby.user.username"]);
    ...
    const qMasterNumber = new Parse.Query("masternumber");
    const masterNumber = await qMasterNumber.select(["prefix", "available"]).get("692dDylmBd").catch((err) =>
    {
    ...
    fs.readFile(__dirname + '/templates/NewWoEmail.html', 'utf8', (err, html) => 
    {
        ...
        let replacements = {
            ...
            woRequester : wo.requester && `${wo.requester.firstname} ${wo.requester.lastname} [${wo.requester.user.username}]`,
            woAssignTo : wo.assignto && `${wo.assignto.firstname} ${wo.assignto.lastname} [${wo.assignto.user.username}]`,
            woCreateBy : wo.createdby && `${wo.createdby.firstname} ${wo.createdby.lastname} [${wo.createdby.user.username}]`
        };
...
Parse.Cloud.define("update_wo", async (request) => {
    ...
    fs.readFile(__dirname + '/templates/UpdateWoEmail.html', 'utf8', (err, html) => 
    {
        let template = handlebars.compile(html);
